 ...
 *           optionally (non-)inheritable. Named pipes can ve read
...
mkNamedPipe (HANDLE* pHandleIn, BOOL isInheritableIn,
             HANDLE* pHandleOut, BOOL isInheritableOut)
{
#if 1 //defined(__IO_MANAGER_WINIO__)
    ...
    /* Set some optimization flags to makethe I/O manager operate more
    ...
    int flags, bool useJobObject, HANDLE *hJob, bool asynchronous)
{
    ...
    if (!setStdHandleInfo (&sInfo.hStdError, _stderr, &hStdErrorRead,
                           &hStdErrorWrite, defaultStdError, TRUE, TRUE,
                           asynchronous))
      goto cleanup_err;
